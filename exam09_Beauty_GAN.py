...
#
# fig, ax = plt.subplots(1, figsize=(10, 16))
# x, y, w, h = dets[0].left(), dets[0].top(), dets[0].width(), dets[0].height()
# rect = patches.Rectangle((x, y), w, h,
#                          linewidth=2, edgecolor='b', facecolor='None')
# ax.add_patch(rect)
# ax.imshow(img_result)
# plt.show()
# # 5개의 랜드마크 찾기(눈 양쪽 끝과 코) # 위 모델 쉐임 프래딕터 5 또는 68을 써서 랜드마크를 얻을 수 있다.
...
def align_faces(img):
    ...
    for detection in dets:
        s = shape(img, detection)
        objs.append(s)
    faces = dlib.get_face_chips(img, objs, size=256, padding=0.35) # 패딩을 주면 여유있게 잘라줌
    return faces
...
img1 = dlib.load_rgb_image('./imgs/13.jpg')
